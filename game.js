...
class Game {
  VERSION = { num: '0.16.4', build: 298, channel: 'alpha', date: '2026-09-05', codename: 'Night Log Strip' };
  SAVE_VER = 16;
  KEY = 'afterglow.save';
  ...
  MIGRATIONS = {
    ...
    4(g) {
      ...
      for (const goal of this.GOALS) {
        if (goal.check.call(this, view)) g.goals.push(goal.id);
      }
    },
  ...
  CHANGELOG = [
    ...
    { v: '0.11.9', date: '2026-08-15', codename: 'Neon Zero', notes: [
      'SECOND PRESTIGE LAYER (REPLAY_ROADMAP.md §8): when every prestige perk is maxed, every manager is hired, and both clubs are unlocked, the Perks panel gains a distinct "Sell the franchise" control — a national conglomerate buys your whole operation. Selling resets EVERYTHING (both clubs, Legacy, perks, research, Clout, managers, crew, challenges, run counters) in exchange for Renown, a new permanent meta-currency (floor(√lifetime Legacy + prestiges/3)). Renown and lifetime Renown never wipe; achievements and Brand ranks persist — the reason to build and sell again. Two-click armed confirm with a reset-scope preview, persist-before-replace like prestige, and a Renown readout after the first sale. SAVE_VER bumped to 10; v9 saves migrate with the new fields defaulted.'
    ] },
  ...
  // Extracted 10Hz sim tick callback — single source for init() and pageshow re-arm.
  // Carries the Math.min(dt, this.MAX_DT) clamp on both catchUp and liveStep branches,
  // fixing the latent divergence where pageshow omitted the clamp on liveStep.
  startTickTimer() {
  ...
  ensureOwnerLifecycle() {
    ...
    window.addEventListener('pageshow', () => {
      ...
      // timer callback via startTickTimer() if still owner.
  ...
  noteGoals(g, opts = {}) {
    ...
    if (!goal || typeof goal.check !== 'function' || !goal.check.call(this, view)) return;
...
// Catalog data lives in catalogs.js (loaded before this file); attach it to the
// prototype so every `this.BUILDINGS` / `this.ACHIEVEMENTS` … call site resolves
// through inheritance exactly as when these were instance fields.
Object.assign(Game.prototype, window.AfterglowCatalogs);
