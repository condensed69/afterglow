...
class Game {
  ...
  ACHIEVEMENTS = [
    ...
    { id: 'brand_max', name: 'Brand Portfolio', desc: 'Max all 5 Brand perks', check: g => (g.brand && this.BRAND_PERKS.every(p => (g.brand[p.id] || 0) >= p.max)), reward: { legacy: 5 } },
  ...
  renderVals() {
    ...
    const tabs = tabDefs.map(t => ({
      ...
      style: {
        flex: '0 0 auto', padding: '11px 12px', background: this.state.tab === t.id ? '#170e22' : 'transparent',
        ...
        scrollSnapAlign: 'start', whiteSpace: 'nowrap', minWidth: 'max-content'
      }
    }));
...
<div style="height:100vh;height:100dvh;display:grid;grid-template-rows:auto auto 1fr auto;grid-template-columns:minmax(0,1fr);background:radial-gradient(1200px 700px at 50% -10%,#1a0e26 0%,#07050c 62%);overflow:hidden">
  ...
  <main data-scroll="main" class="shell-grid">
    ...
    <aside class="sys-col" style="border-left:1px solid #2a1738;background:#0a0611;display:grid;grid-template-rows:auto auto minmax(0,1fr);min-height:0">
      <div style="display:flex;overflow-x:auto;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;border-bottom:1px solid #2a1738;background:#0d0814;padding:0 12px">${tabRows}</div>
