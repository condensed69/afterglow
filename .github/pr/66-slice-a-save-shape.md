...
- `node economy.test.mjs` — **218 passed, 0 failed** (was 209; +9: v8→v9 migration shape, `club()` resolution/fallback/activeClub routing incl. inherited-key rejection, fresh() v9 partition, empty-clubs-map rejection, prestige club preservation, activeClub resolves to an existing club when `main` is absent, reserved club IDs (`__proto__`) rejected fail-closed, plus the armed `SPLIT_TEST`)
