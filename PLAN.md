...
- **Second location** — design pass **done**: `SECOND_LOCATION.md`. Save-shape groundwork shipped in 0.11.0 (SAVE_VER 9: run state moved into `g.clubs.main`, v8→v9 migration, `club()` accessor threaded through the sim); **second-room gameplay shipped in 0.11.1 (Slice B: gate + unlock modal, header switcher, cap-aware crew rebalance, ledger room label)**; the second-room pacing scenario (`pacing2.mjs`) remains deferred (Slice C). `franchise` removal in 1.9 keeps the door open.
